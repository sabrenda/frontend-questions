...
При конфликте стилей в CSS применяются те, которые имеют наибольший приоритет. Приоритет определяется:

1. Правила с `!important` - имеют самый высокий приоритет
2. **Inline-стили** (`style=""`)
3. **Идентификаторы (ID)** (`#id`)
4. **Классы**, (`.class`), **атрибуты** (`input[type="text"]`) и **псевдоклассы** (`a:hover`)
5. **Теги** (`p {...}`) и **псевдоэлементы** (`h1::before {...}`)
6. **Универсальный селектор (`*`)** - имеют самый низкий приоритет
