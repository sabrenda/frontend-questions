...
1. что такое реативность
2. Чем React Native отличается от ReactJS?
3. Как вы отлаживаете приложения React и какие инструменты можете использовать?
4. Когда вам использовать Class Component вместо Functional Component? Если ваш компонент имеет состояние или содержит используемые методы значимые для компонента, то используйте Class component. В других случаях используйте Functional component.
5. Метод Render в реакте
6. В чем разница между классовыми компонентами и функциональными?
7. Что такое строгий режим и зачем?
8. В чем разница между useEffect и componentDidMount?
9. В чем разница между constructor и getInitialState?
---
...
|3| [Какие методы HTTP запросов вы знаете ](#other3)
...
|8| [Что такое Git? ](#other8)
|9| [Команды Git? ](#other9)
...
|13| [На каких 3 принципах базируется ООП? Расскажите кратко о каждом. ](#other13)
|14| [Какие паттерны проектирования вы чаще всего используете? ](#other14)
|15| [Для чего нужен package.json? ](#other15)
|16| [Что такое Cross-Origin Resource Sharing (CORS)?](#other16)
|17|Какие существуют основные принципы ООП?

В чем семантическое отличие PUT и PATCH
метод OPTION (пред запрос, получает политики, доступные методы)
...
JS паттерны (Singleton, Factory, Адаптер)
БЭМ (Блок-Элемент-Модификатор)
REST api vs RESTFullAPi
unit-test
IIFE Самовызывающиеся функции JavaScript (function () { //body }());
Function Expression let sayHi = function() { alert( "Привет" ); };
Function declarations Объявление функции вводит идентификатор , обозначающий функцию, и, при необходимости, указывает типы параметров функции ( прототип ). Объявления функций (в отличие от определений ) могут появляться как в области блока, так и в области файла.
pwa (service-Workers)
...
Компонент высшего порядка (Higher-Order Component, HOC) - это паттерн разработки, используемый в библиотеке React для повышения переиспользования логики компонентов. HOC является функцией, которая принимает компонент и возвращает новый компонент с дополнительной функциональностью.

HOC предоставляет способ обернуть один компонент вокруг другого, добавляя или изменяя его поведение. Он позволяет абстрагировать общую логику, такую как обработка состояния, манипуляция данными или добавление контекста, и применить ее к нескольким компонентам без дублирования кода.

Примером HOC может быть компонент, обрабатывающий аутентификацию пользователей. Он может принимать компонент и проверять, аутентифицирован ли пользователь. В зависимости от результата проверки, HOC может рендерить либо обернутый компонент, либо компонент для входа в систему.

Преимущества использования HOC включают повышение переиспользуемости кода, разделение общей логики от презентационной части компонентов, улучшение читаемости и поддерживаемости кода. HOC также является одним из фундаментальных паттернов композиции компонентов в React.

Однако в React с появлением хуков (hooks), таких как `useEffect` и `useContext`, появились альтернативные способы реализации функциональности, которую ранее обеспечивали HOC. Хуки позволяют более прямую манипуляцию состоянием и контекстом компонента без необходимости использования HOC.
...
## 33. Что такое порталы в React?
  ...
  Порталы (Portals) в React предоставляют возможность рендерить дочерние компоненты вне их обычной иерархии DOM-элементов. Они позволяют разработчикам создавать компоненты, которые могут быть вставлены в произвольное место внутри DOM-структуры приложения.

С помощью порталов можно решить такие задачи, как:

1. Рендеринг компонентов вне корневого элемента приложения: По умолчанию компоненты в React рендерятся внутри корневого элемента, определенного в методе `ReactDOM.render()`. С помощью порталов можно рендерить компоненты в другие элементы вне основной иерархии.

2. Создание модальных окон и всплывающих сообщений: Порталы позволяют рендерить модальные окна и всплывающие сообщения вне основного потока компонентов, что может быть полезно для управления позиционированием и слоистостью этих элементов.

Для создания портала в React необходимо использовать компонент `ReactDOM.createPortal()`. Этот метод принимает два аргумента: компонент, который нужно отрендерить, и DOM-элемент, в который он должен быть вставлен. Например:
...
В приведенном примере компонент `Modal` рендерит своих детей вне основного элемента `root` и вставляет их в элемент с идентификатором `modal-root`, который должен существовать в HTML-структуре страницы.

Порталы в React предоставляют удобный способ управления компонентами, которые должны быть размещены вне основной иерархии компонентов, и позволяют более гибко управлять размещением и взаимодействием с элементами в DOM-структуре приложения.
