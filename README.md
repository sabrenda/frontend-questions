...
## 5. Что такое прототип объекта в JavaScript? Prototype
...
**`e.stopImmediatePropagation()`** делает то же самое, что и stopPropagation, но, в том числе, предотвращает вызов обработчиков события, которые были установлены на этом же элементе.
...
## 73. Что такое мемоизация и кеширование
...
## 74. Почему typeof массива === object
...
## 75. Отличие Event.target от event.currentTarget
...
## 77. Дескрипторы свойств объекта
...
## 81. IIFE Самовызывающиеся функции
...
- **`Constructor`**: Функция-конструктор, с которой проверяется связь объекта.

### Как работает `instanceof`

`instanceof` проверяет, находится ли `Constructor.prototype` в цепочке прототипов объекта `object`. Если да, то оператор возвращает `true`; иначе — `false`.
...
В этом примере `john` является экземпляром класса `Person`, поэтому `john instanceof Person` возвращает `true`. Поскольку все объекты в JavaScript наследуют от `Object`, `john instanceof Object` также возвращает `true`. Однако `john` не является массивом, поэтому `john instanceof Array` возвращает `false`.

### Применение

1. **Проверка типа объекта**:
   - `instanceof` часто используется для проверки типа объекта перед выполнением определенных операций, чтобы избежать ошибок, связанных с несовместимыми типами данных.

2. **Работа с наследованием**:
   - При работе с объектами, созданными с использованием прототипного наследования, `instanceof` помогает определить, был ли объект создан с использованием определенного конструктора или его наследников.

3. **Безопасность кода**:
   - `instanceof` может использоваться для проверки типа перед вызовом методов, чтобы убедиться, что объект поддерживает необходимые методы и свойства.
...
## 85. Как сделать обьект неизменяемым, методы Object.freeze, Object.seal, Object.defineProperty
...
## 87. Как работает ключевое слово new
