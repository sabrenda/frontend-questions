...
|32| [Что такое каррирование `функция карри`?](#32)|
...
|104| [Что такое мемоизация и кеширование?](#104)|
...
### Основные особенности «strict» режима:

1. **Запрет на использование необъявленных переменных**:
   В обычном режиме JavaScript автоматически создает глобальные переменные при присвоении значений необъявленным переменным. В строгом режиме это вызывает ошибку.
   ```javascript
   "use strict";
   x = 10; // Ошибка: переменная не объявлена
   ```

2. **Запрет на дублирование свойств объекта**:
   В строгом режиме нельзя иметь несколько свойств с одинаковыми именами в объекте.
   ```javascript
   "use strict";
   const obj = { x: 1, x: 2 }; // Ошибка: дублирование имени свойства
   ```

3. **Запрет на дублирование параметров функции**:
   В строгом режиме нельзя иметь несколько параметров с одинаковыми именами в функции.
   ```javascript
   "use strict";
...
4. **Ограничение использования `this` в функциях**:
   В строгом режиме, если `this` используется в функции, вызванной без контекста (без объекта), оно не ссылается на глобальный объект (`window` в браузерах). Вместо этого `this` будет `undefined`.
   ```javascript
   "use strict";
   function showThis() {
     console.log(this); // `this` будет `undefined`, а не `window`
   }
   ```

5. **Запрет на удаление (`delete`) переменных или функций**:
   В строгом режиме нельзя удалять переменные, функции или аргументы.
   ```javascript
   "use strict";
   var x = 1;
   delete x; // Ошибка
...
6. **Ограничения для зарезервированных слов**:
   Строгий режим запрещает использование некоторых зарезервированных слов для будущих версий JavaScript, таких как `implements`, `interface`, `let`, `package`, `private`, `protected`, `public`, `static`, и `yield`.

Этот режим можно включить как для всего файла, так и для отдельной функции.
...
>
> <details>
> <summary>реализация прототипа Promise.all() в js:</summary>
>   	
>   ```js
...
>   
> <details>
> <summary>реализация прототипа Promise.race() в js:</summary>
> 	
>   ```js
...
>   
> <details>
> <summary>реализация прототипа Promise.allSettled() в js:</summary>
> 	
>   ```js
...
>   
> <details>
> <summary>реализация прототипа Promise.any() в js:</summary>
> 	
>   ```js
...
## 20. Как работает ключевое слово `this`?
...
### 4. **Внутри конструктора (классы и функции-конструкторы)**
...
- **`call()`**: вызывает функцию с указанием конкретного объекта в качестве `this` и принимает аргументы через запятую.
...
## 58. В чем разница между `host и native` объектами?
...
## 61. Распространение события и фазы
...
## 62. Всплытие и погружение событий. `bubbling, capturing`
...
## 63. `e.preventDefault()` и `e.stopPropagation()`, `e.stopImmediatePropagation` для чего нужны?
...
## 64. Что такое делегирование событий в JS?
...
## 65. Отличие `Event.target от event.currentTarget`
...
## 66. Что такое `throttle` и `debounce`?
...
## 67. Как остановить интервал? `clearInterval(intervalId)`
...
## 68. Что такое `requestAnimationFrame`?
...
## 69. Что такое `requestIdleCallback`?
...
## 72. Какие есть способы для работы с запросами в React?
...
## 73. Как сервер может запретить чтение cookie из браузера? `флаг HttpOnly`
...
## 74. методы массивов в JavaScript
...
## 77. Какие способы хранения данных в браузере вы знаете? `LocalStorage, sessionStorage, Cookies, IndexedDB`
...
## 78. В чем отличие sessionStorage от localStorage?
...
## 79. Что будет, если добавить скрипт перед тегом body?
...
## 80. Разница между `<script>`, `<script async>` и `<script defer>`
...
## 83. `WeakSet и WeakMap`: отличие от `Map и Set`
...
## 86. Что такое регулярные выражения? `Regex`
...
## 95. Различия между старым js и новым js
...
## 97. Структуры данных и Сложность алгоритмов
...
## 98. Как дебажить приложение и причины утечек памяти?
...
## 100. Какие особенности есть у JavaScript?
...
## 104. Что такое мемоизация и кеширование
