...
1. **Тип `any`**:
   ...
   **Пример**:
   ...
   Как видно из примера, TypeScript не выдает ошибок, даже если метод `foo()` не существует у значения типа `any`. Это может стать причиной проблем в дальнейшем, так как проверки отсутствуют.
